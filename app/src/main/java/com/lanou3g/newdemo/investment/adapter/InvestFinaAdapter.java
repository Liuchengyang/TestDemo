 ...
 */
public class InvestFinaAdapter extends BaseAdapter {

    private Context context ;
    private InvestFinanBean bean;

    public InvestFinaAdapter(Context context) {
        this.context = context;
    }

    public void setBean(InvestFinanBean bean) {
        this.bean = bean;
    }
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ViewHolderFina viewHolder =null;
        if (view==null) {
            view = LayoutInflater.from(context).inflate(R.layout.invest_all_fragment_list_item,viewGroup,false);
            viewHolder =new ViewHolderFina(view);
            view.setTag(viewHolder);
        }else {
            viewHolder = (ViewHolderFina) view.getTag();
        }
        Picasso.with(context).load(bean.getData().getData().get(i).getCompany_logo()).into(viewHolder.imageViewlogal);
        viewHolder.textViewcompanyname.setText(bean.getData().getData().get(i).getCompany_name());
        viewHolder.textViewsingmeting.setText(bean.getData().getData().get(i).getCompany_brief());
        Picasso.with(context).load(bean.getData().getData().get(i).getFile_list_img()).into(viewHolder.imageViewbigpic);
        viewHolder.textViewsecondcreat.setText(bean.getData().getData().get(i).getCf_advantage().get(0).getAdname());
        viewHolder.textViewthirdteamnumber.setText(bean.getData().getData().get(i).getCf_advantage().get(1).getAdname());
        viewHolder.textViewleadname.setText(bean.getData().getData().get(i).getLead_name());
    ...
    class ViewHolderFina{
        ...
        public ViewHolderFina(View view){
